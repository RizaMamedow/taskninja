...
const ListItem = ({task, deleteTask, toggleCompleteTask}: IListItem) => {
    ...
    return (
        <ListGroup.Item variant={task.isCompleted ? "secondary" : 'white'} className="smooth-transition">
            <div className="task d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex flex-column justify-content-between align-items-end" style={{minWidth: '220px'}}>
                    <div className="d-flex flex-row">
                        {!task.isCompleted ?
                            <Button variant="success" className="pt-1" onClick={() => { toggleCompleteTask(task.id) }}><FaCheck/> Complete</Button>
                            : <Button variant="secondary" className="pt-1" onClick={() => { toggleCompleteTask(task.id) }}><FaMinus/> Uncomplete</Button>}
                        <Button variant="danger" className="ms-1 pt-1" onClick={() => { deleteTask(task.id) }}><FaTrash/> Remove</Button>
                    </div>
